######################################################################
#
#  KDE and EXPECTATION MAXIMIZATION for WINE dataset.
#
######################################################################
...
# the z-score of a value x is z=(x-mu)/sigma
...
# covariance, and make sure it is positive definite; 
# Added a regularization for sigma to make sure
# that is not rank deficient and also positive definite.
sigma = np.zeros((2, 2, K))
for i in range(0, K):
    tmp = np.random.randn(2, 2)
    sigma[:, :, i] = tmp.dot(tmp.T) + np.power(10.0, -3) * np.eye(2)
